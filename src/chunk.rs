...
impl TryFrom<&[u8]> for Chunk {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let length_bytes: [u8; 4] = value[0..4].try_into().unwrap();
        let length = u32::from_be_bytes(length_bytes);

        let chunk_type_bytes: [u8; 4] = value[4..8].try_into().unwrap();
        let chunk_type = ChunkType::try_from(chunk_type_bytes).unwrap();

        let data = Vec::from(&value[8..8 + length as usize]);

        let crc = u32::from_be_bytes(
            value[8 + length as usize..(8 + length + 4) as usize]
                .try_into()
                .unwrap(),
        );
...
impl Chunk {
    ...
    pub fn data_as_string(&self) -> Result<String, ()> {
        String::from_utf8(self.data.clone()).map_err(|_| ())
    }
